...
SELECT A.descricao, AVG(L.preco) AS media_preco 
...
GROUP BY L.titulo HAVING COUNT(AL.autor_codigo) >= 2;
...
SELECT E.nome, MAX(L.preco) AS preco_max, MIN(L.preco) AS preco_min, AVG(L.preco) AS preco_medio 
FROM livro L INNER JOIN editora E ON L.editora_codigo = E.codigo 
WHERE L.assunto_codigo IN (1, 2, 3) GROUP BY E.nome;
...
FROM livro L INNER JOIN editora E ON L.editora_codigo = E.codigo 
GROUP BY E.nome, L.assunto_codigo ORDER BY quantidade DESC;
...
-- 54. Descrição de todos os assuntos e os títulos dos livros de cada um. Quando não existir um livro associado ao assunto, escrever o texto ‘Sem publicações’
...
WHERE L.assunto_codigo IN 
    (SELECT assunto_codigo FROM livro GROUP BY assunto_codigo HAVING AVG(preco) >= 40.00);
...
SELECT titulo FROM livro WHERE editora_codigo NOT IN 
	(SELECT codigo FROM editora WHERE nome LIKE "%Books%")
    AND assunto_codigo IN
    (SELECT codigo FROM assunto WHERE descricao LIKE "%Banco de Dados%");
