...
export function average(lista) {
    ...
    let media = (parseFloat((somatorio / lista.length).toFixed(2)));
...
export function map_Fraction(lista, fracao) {
    let nova_Lista = [];
    
    for (let numero of lista) {
        let novo_Numero = (numero * fracao[0]) / fracao[1];
        nova_Lista.push(parseFloat(novo_Numero.toFixed(2)));
...
export function map_NegativesNumbers(lista) {
    let nova_Lista = [];
    
    for (let i = 0; i < lista.length; i++) {
        if(get_negative_number(lista[i])) {
            print(`\n\r\tNUMERO NEGATIVO ENCONTRADO ${lista[i]}`);
            let novo_Numero = get_number(`\n\r\tDIGITE UM NOVO NUMERO POSITIVO: `);
            lista[i] = novo_Numero;
            nova_Lista.push(lista[i]);
        }else {
            nova_Lista.push(lista[i]);
