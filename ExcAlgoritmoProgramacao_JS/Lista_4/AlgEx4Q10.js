...
function main() {
    const numero_containers = Number(question(`Digite o numero de containers que seram transportados: `));
    let volume_containers = Calcular_VolumeContainer(numero_containers);

    const numero_passageiros = Number(question(`Digite o numero de passageiros do voo: `));
    ...
    if(peso_decolagem <= 500000) {
        console.log(`A decolagem está liberada, prossiga com a viagem.`);
    }else {
        console.log(`A decolagem está negada, retorne ao Angar.`);
    }
}
...
function Calcular_VolumeContainer(numero) {
    ...
    while(total !== 0) {
        let volume_individual = Number(question(`Digite o peso do conainer n°${total}: `));
...
function Calcular_VolumePassageiro(numero) {
    ...
    while(total !== 0) {
        let numero_volume = Number(question(`Digite quantas baganens voce está levando passageiro n°${total}: `));
